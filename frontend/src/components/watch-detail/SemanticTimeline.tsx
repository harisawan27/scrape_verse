      ...
      {safeEvents.map((event, idx) => {
        ...
        return (
          <div key={event.id || idx} className="relative group">
            ...
            <div className="rounded-2xl p-4 bg-space-900/60 border border-space-750 group-hover:border-space-600 transition-colors">
              <div className="flex items-center justify-between gap-2 mb-1.5">
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${typeInfo.color}`}
                  >
                    {typeInfo.label}
                  </span>
              ...
              {/* Price Delta Pill */}
              {hasPriceDiff && (
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-space-950/80 border border-space-750 text-xs font-mono">
                  ...
                  {isDrop && (
                    <span className="inline-flex items-center gap-1 text-[11px] font-bold text-radar-emerald bg-radar-emerald/10 px-1.5 py-0.5 rounded">
                      ...
                      <span>
                        -{Math.round(((prevPrice - currPrice) / prevPrice) * 100)}%
                      </span>
                    </span>
                  )}
                </div>
